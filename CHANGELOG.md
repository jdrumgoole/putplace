...
### Fixed
- `routers/pages.py` now redirects the three bare email-link paths regstack
  hardcodes (`/verify`, `/reset-password`, `/confirm-email-change`) to the
  matching pages under regstack's `ui_prefix` (`/account/*`). Without this,
  the link in every verification / password-reset / email-change email
  resolves to a putplace 404, because regstack builds the URL as
  `f"{base_url}/verify?token=..."` with no UI prefix. Belt-and-braces with
  the parallel upstream fix being proposed against regstack itself.
...
#### Maintenance Release (pp_gui_client)
- **Updated server compatibility**: Now compatible with putplace-server v0.10.2+
  - Updated dependency documentation
  - Rebuilt packages for distribution
  - No functional changes to the client
