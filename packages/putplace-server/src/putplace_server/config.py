...
User-account configuration (JWT, OAuth, email, registration toggle, admin
bootstrap, ...) lives entirely in regstack now — see
:mod:`putplace_server.regstack_integration` and ``REGSTACK_*`` env vars.
This module only carries settings that putplace itself still owns:
MongoDB connection, API metadata, storage backend, AWS S3, CORS.
"""
...
def find_config_file() -> Optional[Path]:
    ...
    if env_config:
        ...
        import logging
        logging.warning(
            f"PUTPLACE_CONFIG set to {env_config} but file not found, "
...
class Settings(BaseSettings):
    """Application settings (MongoDB, API, storage, AWS, CORS only).

    User-account settings (JWT, OAuth, email templates, registration) are
    owned by regstack and read from ``REGSTACK_*`` env vars or
    ``regstack.toml`` — see :mod:`putplace_server.regstack_integration`.
    """
